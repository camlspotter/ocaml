...
(* This is defined in Untypeast but not linked here *)
...
let resolve_overloading exp lidloc path = 
  ...
  let rec find_candidates (path : Path.t) mty =
    ...
    | Sig_value (id, _vdesc) when Ident.name id = name -> 
        let lident = Longident.Ldot (lident_of_path path, Ident.name id) in
  ...
  let lid_opt = match path with
    | Path.Pident _ -> None
    | Path.Pdot (p, _, _) -> Some (lident_of_path p)
    | Path.Papply _ -> assert false
  in
  ...
  | [path, vdesc] -> 
      ...
      { exp with 
        exp_desc = Texp_ident (path, {lidloc with Asttypes.txt = lident_of_path path}, vdesc);
