...
let type_open ?toplevel ovf env loc lid hidings =
  let (path, mty) = Typetexp.find_module env loc lid.txt in
  let sg = extract_sig_open env loc mty in
  path, Env.open_signature ~loc ?toplevel ovf path sg hidings env
...
and approx_sig env ssg =
  ...
  | item :: srem ->
      ...
      | Psig_open (ovf, lid, hidings) ->
          let (path, mty) = type_open ovf env item.psig_loc lid hidings in
          approx_sig mty srem
      | Psig_include smty ->
          let mty = approx_modtype env smty in
  ...
  let rec transl_sig env sg =
    ...
    | item :: srem ->
        ...
        | Psig_modtype(name, sinfo) ->
            ...
            final_env
        | Psig_open (ovf, lid, hidings) ->
            let (path, newenv) = type_open ovf env item.psig_loc lid hidings in
            let (trem, rem, final_env) = transl_sig newenv srem in
            mksig (Tsig_open (ovf, path,lid,hidings)) env loc :: trem,
            rem, final_env
        | Psig_include smty ->
            let tmty = transl_modtype env smty in
            let mty = tmty.mty_type in
  ...
  let rec type_struct env sstr =
    ...
    | Pstr_modtype(name, smty) ->
        ...
        (item :: str_rem,
         ...
         final_env)
    | Pstr_open (ovf, lid, hidings) ->
        let (path, newenv) = type_open ovf ~toplevel env loc lid hidings in
        let item = mk (Tstr_open (ovf, path, lid, hidings)) in
        let (str_rem, sig_rem, final_env) = type_struct newenv srem in
        (item :: str_rem, sig_rem, final_env)
    | Pstr_class cl ->
         List.iter
